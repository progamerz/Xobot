...
public class BoltMethods {
	public static boolean canMakeBolts() {
		return Inventory.Contains(Data.mainIng) && Inventory.Contains(Data.secondaryIng)
				&& !busy();
	}

	public static void doBolts() {
	 ...
	 private static boolean busy(){
	        ...
	        while(t.getElapsed() < 2120){
	            if(Player.getMyPlayer().getAnimation() == 16402 || Inventory.getAll().length < startCount){
	                return true;
	            }
	            Time.sleep(40, 100);
	        }
